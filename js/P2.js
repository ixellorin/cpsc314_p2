...
var SUN_ROTATION_SPEED = 1;
var EARTH_ORBITAL_VELOCITY = .1;
var MERCURY_ORBITAL_VELOCITY = EARTH_ORBITAL_VELOCITY / .24;
var VENUS_ORBITAL_VELOCITY = EARTH_ORBITAL_VELOCITY / .61;
var MARS_ORBITAL_VELOCITY = EARTH_ORBITAL_VELOCITY / 1.88;
var JUPITER_ORBITAL_VELOCITY = EARTH_ORBITAL_VELOCITY / 5.9;
var SATURN_ORBITAL_VELOCITY = EARTH_ORBITAL_VELOCITY / 14.4;
var URANUS_ORBITAL_VELOCITY = EARTH_ORBITAL_VELOCITY / 30.7;
var NEPTUNE_ORBITAL_VELOCITY = EARTH_ORBITAL_VELOCITY / 40.7;
...
var distance_from_sun = [23, 25, 34, 51, 80, 120, 130, 140];
...
for (i = 0; i < anchors.length; i++) {
	var geometry = new THREE.RingGeometry(distance_from_sun[i], distance_from_sun[i] + 1, 16, 1, 0, Math.PI * 2);
...
{	
	if(!freeze_state) {
		...
		mercury_anchor.rotateOnAxis(Y_AXIS, MERCURY_ORBITAL_VELOCITY);
		venus_anchor.rotateOnAxis(Y_AXIS, VENUS_ORBITAL_VELOCITY);
		earth_anchor.rotateOnAxis(Y_AXIS, EARTH_ORBITAL_VELOCITY);
		mars_anchor.rotateOnAxis(Y_AXIS, MARS_ORBITAL_VELOCITY);
		jupiter_anchor.rotateOnAxis(Y_AXIS, JUPITER_ORBITAL_VELOCITY);
		saturn_anchor.rotateOnAxis(Y_AXIS, SATURN_ORBITAL_VELOCITY);
		uranus_anchor.rotateOnAxis(Y_AXIS, URANUS_ORBITAL_VELOCITY);
		neptune_anchor.rotateOnAxis(Y_AXIS, NEPTUNE_ORBITAL_VELOCITY);
...
function onKeyDown(event) {
  ...
  if(keyboard.eventMatches(event, "p")) {
	  scoutship_state = !scoutship_state;
	  mothership_state = false;
  }
  if(keyboard.eventMatches(event, "m") || keyboard.eventMatches(event, "l") || keyboard.eventMatches(event, "r") || keyboard.eventMatches(event, "g")) {
  ...
  if(keyboard.eventMatches(event, "g")) {
	...
	if(lookat_state && (mothership_state || scoutship_state) && keyboard.eventMatches(event, "f")) {
		if(keyboard.eventMatches(event, "shift+f")) {
			mothership_state? camera_MotherShip.up.z -= stepsiz : camera_ScoutShip.up.z -= stepsize;
		} else {
...
// console.log("mothership state: " + mothership_state + " scoutship state: " + scoutship_state + " lookat state: " + lookat_state + " relative state:" + relative_state + " geosynchronous state: " + geosynchronous_state);
